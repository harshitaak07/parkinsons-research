...
Original file is located at
    https://colab.research.google.com/drive/1VAjCEyOrJ4SYCQn6ZMgQKPr0DBtPUuWP
"""

import pandas as pd
df_pyarrow5 = pd.read_parquet('sleep.parquet', engine='pyarrow')
print(df_pyarrow5['EVENT_ID'].head())
print(df_pyarrow5['EVENT_ID'].isnull().sum())

print(df_pyarrow5[['PATNO', 'EVENT_ID']].isnull().sum())

symptom_cols_cog2 = [col for col in df_pyarrow5.columns if col not in ['PATNO', 'visit_phase']]
for col in symptom_cols_cog2:
    df_pyarrow5[col] = pd.to_numeric(df_pyarrow5[col], errors='coerce')


# Now group and calculate mean prevalence by 'stage'
prevalence = (
    df_pyarrow5.groupby('visit_phase')[symptom_cols_cog2]
    .mean()
    .transpose()
)
...
df_pyarrow5['PATNO'] = df_pyarrow5['PATNO'].astype(str)
df_pyarrow5['visit_phase'] = df_pyarrow5['visit_phase'].astype(str)

df_pyarrow5agg = df_pyarrow5.groupby(['PATNO', 'visit_phase'])[selected_symptoms].mean().reset_index()

pivoted = {
    symptom: df_pyarrow5agg.pivot(index='PATNO', columns='visit_phase', values=symptom).fillna(0)
    for symptom in selected_symptoms
}

arrays = [pivoted[s].to_numpy() for s in selected_symptoms]
autoencoder_input_cog5 = np.stack(arrays, axis=2)

print(autoencoder_input_cog5.shape)
df_pyarrow5['PATNO'] = df_pyarrow5['PATNO'].astype(str)
df_pyarrow5['visit_phase'] = df_pyarrow5['visit_phase'].astype(str)
...
import numpy as np
np.save('autoencoder_input_cog5.npy', autoencoder_input_cog5)
