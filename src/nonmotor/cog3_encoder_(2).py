# -*- coding: utf-8 -*-
"""cog3-encoder (2).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1RZ2n5tRvPQj988FTd_owJ4zmd1paGjmn
"""

import pandas as pd
df_pyarrow3 = pd.read_parquet('df2_Selected.parquet', engine='pyarrow')
print(df_pyarrow3['EVENT_ID'].head())
print(df_pyarrow3['EVENT_ID'].isnull().sum())

print(df_pyarrow3[['PATNO', 'EVENT_ID']].isnull().sum())

df_pyarrow3.columns

symptom_cols_cog2 = [col for col in df_pyarrow3.columns if col not in ['PATNO', 'visit_phase']]
for col in symptom_cols_cog2:
    df_pyarrow3[col] = pd.to_numeric(df_pyarrow3[col], errors='coerce')


# Now group and calculate mean prevalence by 'stage'
prevalence = (
    df_pyarrow3.groupby('visit_phase')[symptom_cols_cog2]
    .mean()
    .transpose()
)
...
df_pyarrow3['PATNO'] = df_pyarrow3['PATNO'].astype(str)
df_pyarrow3['visit_phase'] = df_pyarrow3['visit_phase'].astype(str)

df_pyarrow3agg = df_pyarrow3.groupby(['PATNO', 'visit_phase'])[selected_symptoms].mean().reset_index()

pivoted = {
    symptom: df_pyarrow3agg.pivot(index='PATNO', columns='visit_phase', values=symptom).fillna(0)
    for symptom in selected_symptoms
}

arrays = [pivoted[s].to_numpy() for s in selected_symptoms]
autoencoder_input_cog3 = np.stack(arrays, axis=2)

print(autoencoder_input_cog3.shape)
df_pyarrow3['PATNO'] = df_pyarrow3['PATNO'].astype(str)
df_pyarrow3['visit_phase'] = df_pyarrow3['visit_phase'].astype(str)
...
import numpy as np
np.save('autoencoder_input_cog3.npy', autoencoder_input_cog3)
