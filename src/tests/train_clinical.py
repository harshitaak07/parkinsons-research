"""
Training script using clinical motor severity labels (UPDRS Part III).
Replaces synthetic labels with real clinical data from motor_labels.csv.
"""
...
def main():
    ...
    axivity_info = get_modality_info(axivity_labeled)
    opals_info = get_modality_info(opals_labeled)
    print(f"Axivity info: {axivity_info}")
    print(f"Opals info: {opals_info}")
    
    # Choose primary modality for training (use Axivity if available, otherwise Opals)
    ...
    try:
        ...
        non_motor_features = features  # Use same features as proxy
    
    # Normalize features
    scaler = StandardScaler()
    features_scaled = scaler.fit_transform(features)
    
    # Create time delta features (simplified - could be improved with real time data)
    ...
    print(f"Starting training with {len(train_features)} train samples, {len(val_features)} val samples")
    ...
    print(f"Model saved as 'clinical_model.pth'")
